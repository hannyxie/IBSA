...
class MutmapPage(QTabWidget):
    ...
    def tab1UI(self):
        ...
        self.parent_combox = QComboBox()
        self.parent_combox.addItems(self.mutmap_df_index)
        self.parent_combox.currentIndexChanged.connect(self.selectparent)
        pool_label = QLabel('pool')
        self.pool_combox = QComboBox()
        self.pool_combox.addItems(self.mutmap_df_index)
        self.pool_combox.currentIndexChanged.connect(self.selectpool)
        ...
        layout = QGridLayout()
        layout.addWidget(parent_label, 0, 0)
        layout.addWidget(self.parent_combox,0, 1)
        layout.addWidget(pool_label, 1, 0)
        layout.addWidget(self.pool_combox, 1, 1)
    ...
    def selectpool(self):
        self.pool_index = self.pool_combox.currentIndex()
    
    def selectparent(self):
        self.parent_index = self.parent_combox.currentIndex()
    ...
    def clean_df(self):
        ...
        parent = df.iloc[:,self.parent_index].str.split(':', expand=True).iloc[:,filed_index]
        parent.columns = ['parent_gt', 'parent_ad','parent_dp']
        pool = df.iloc[:,self.pool_index].str.split(':', expand=True).iloc[:, filed_index]
        pool.columns = ['pool_gt', 'pool_ad','pool_dp']
        df = pd.concat([df,parent,pool], axis= 1)
        #对一些需要的部分进行过滤
        df.dropna(inplace=True)
        df = df.replace(['.|.', '0|0','0|1','1|1','0|2','1|2','2|2'],['./.', '0/0','0/1','1/1','0/2','1/2','2/2'])
        df = df[df['parent_gt'].isin(['0/0','1/1'])]
        df = df[df['pool_gt'] != './.']
        df['parent_dp'] = df['parent_dp'].astype('str').astype('int')
        df['pool_dp'] = df['pool_dp'].astype('str').astype('int')
        df['POS'] = df['POS'].astype('str').astype('int')
        df = df[df['parent_dp']>min_depth]
        df = df[df['parent_dp']<max_depth]
        ...
        df['snp_index']=df.apply(lambda x: get_snp_index(x.pool_gt,x.pool_ad,x.parent_gt), axis = 1)
        df = df[(df['snp_index']>min_snpindex)]
        df.dropna(inplace=True)
        self.df = df[['CHROM', 'POS', 'REF', 'ALT','parent_gt','pool_gt','snp_index']]
    ...
    def slide_window(self):
        ...
        def get_slide_index(chrom,window_start,window_end,df):
            ...
            if data_df.shape[0] > n_snp:
                ...
                slide_index =None
            return slide_index
        df_window['slide_snp_index']=df_window.apply(lambda x: get_slide_index(x.chrom,x.window_start,x.window_end,df),axis = 1)
